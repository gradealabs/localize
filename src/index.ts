...
export class Localize {
  ...
  set locale (value) {
    if (this.isLocaleSupported(value)) {
      ...
      throw Object.assign(
        new Error(`Locale unsupported [${value}]`),
        { code: 'LOCALE_UNSUPPORTED', locale: value }
      )
    }
  }
  ...
  lc (key, context = {}) {
    ...
    if (string) {
      ...
      throw Object.assign(
        new Error(`String not found with key [${key}]`),
        { code: 'STRING_NOT_FOUND', key }
      )
    }
  }
   ...
   */
  interpolateString (string: string, context = {}) {
    if (string.indexOf('#{') >= 0) {
      return string.replace(/#\{([^\}]+)\}/, (_, key) => {
        if (key in context) {
          ...
          throw Object.assign(
            new Error('Context key not found'),
            { code: 'CONTEXT_KEY_NOT_FOUND', key, context }
          )
        }
      })
    } else {
 ...
 */
export class StaticLocalize extends Localize {
  private locales: {}
  ...
  constructor ({ nativeLocale = 'en', locales = null } = {}) {
    ...
    if (locales && Object(locales) === locales) {
      ...
      throw Object.assign(
        new Error('Langs is not an object'),
        { code: 'LANGS_NOT_AN_OBJECT' }
      )
    }
  }
  ...
  lookupString (key: string, defaultString = key): string {
